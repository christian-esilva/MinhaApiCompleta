...
namespace MinhaApp.API.V1.Controllers
{
    ...
    public class ProdutoController : MainController
    {
        ...
        public async Task<ActionResult<ProdutoViewModel>> AdicionarAlternativo(ProdutoImagemViewModel produtoViewModel)
        {
            ...
            var imgPref = $"{Guid.NewGuid()}_";
            if (!await UploadArquivoAlternativo(produtoViewModel.ImagemUpload, imgPref))
                return CustomResponse(produtoViewModel);

            produtoViewModel.Imagem = imgPref + produtoViewModel.ImagemUpload.FileName;
            await _produtoServico.Adicionar(_mapper.Map<Produto>(produtoViewModel));
        ...
        private bool UploadArquivo(string arquivo, string imgNome)
        {
            ...
            {
                NotificarErro("A imagem não pode ser vazia!");
                return false;
            }
